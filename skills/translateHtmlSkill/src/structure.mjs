...
export function structureFindings(source, target, sourcePath, targetPath) {
  ...
  const directives = root => {
    ...
    const visit = node => {
      if (node.type === 'directive')
        found.push(node.data.replace(/^!doctype\s+/i, '!doctype '));
  ...
  if (JSON.stringify(directives(source)) !== JSON.stringify(directives(target)))
    findings.push({
      severity: 'error',
      code: 'document-directives',
      message: 'A document declaration or processing instruction changed or became visible text.'
    });
