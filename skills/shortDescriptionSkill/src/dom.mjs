import { parseDocument, DomUtils } from '../external/html/node_modules/htmlparser2/dist/esm/index.js';

export const parse = text => parseDocument(text);
export const elements = (root, predicate = () => true) => DomUtils.findAll(node => Boolean(node.name) && predicate(node), root.children ?? []);
...
export const meta = (root, name) => first(root, node => node.name === 'meta' && node.attribs.name === name)?.attribs.content;
...
export function textOf(root) {
  ...
  const visit = node => {
    ...
    if (!['script', 'style', 'comment'].includes(node.type)) for (const child of node.children ?? []) visit(child);
  };
  visit(root);
  return parts.join(' ');
}
