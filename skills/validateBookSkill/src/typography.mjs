...
export function compareTypography(profile, document, language='en') {
  const findings=[],mappings=[];
  const normal=s=>normalizeText(s).replaceAll(' ','');
  const pages=profile.pages.map(p=>({...p,joined:normal(p.lines.map(l=>l.text).join(' '))}));
  for(const r of document.records.filter(r=>/^(p|h[1-6]|figcaption)$/.test(r.tag)&&normalizeText(r.text).length>=3)){
    ...
    const text=normal(r.text),matches=pages.filter(p=>p.joined.includes(text));
    if(matches.length!==1||matches[0].joined.indexOf(text)!==matches[0].joined.lastIndexOf(text))continue;
    const p=matches[0],start=p.joined.indexOf(text),end=start+text.length;
    let offset=0;const sizes=[],matchedLines=[];
    for(const line of p.lines){const length=normal(line.text).length;if(offset<end&&offset+length>start){matchedLines.push(line);for(let i=0;i<Math.min(length,100);i++)sizes.push(line.font.sizePt);}offset+=length;}
    const sourcePt=median(sizes);if(!sourcePt)continue;
    const expected=pointsToCssPixels(sourcePt),actual=parseFloat(r.font.size);
    const mapping={selector:r.selector,page:p.page,fontSizePt:sourcePt,expectedCssPx:expected,actualCssPx:actual,sourceFamilies:[...new Set(matchedLines.map(l=>l.font.family))]};const last=matchedLines.at(-1),next=p.lines.find(l=>l.top>last.top+1&&l.font.sizePt===sourcePt);
    ...
    if(sourcePt===profile.bodyPt&&profile.leadingCssPx&&Math.abs(actualLeading-profile.leadingCssPx)>Math.max(1,profile.leadingCssPx*.1))findings.push(issue(language,'line_leading_difference',r.selector,'Distance between lines differs from PDF baseline increments.',{page:p.page,expectedCssPx:profile.leadingCssPx,actualCssPx:actualLeading,repair:{kind:'presentation',selector:r.selector,properties:{'line-height':String(profile.leadingCssPx/expected)}}}));
...
export function typographyActions(profile, document, sourceComparison, {defaultSizePx,justifyPolicy='source',masterTypography=sourceComparison,sourceFontMap={}}={}) {
  ...
  const familyKey=s=>s.replace(/^[A-Z]{6}\+/,'').replace(/[-_ ]?(regular|bold|italic|bolditalic|roman)$/i,'').replace(/[^a-z0-9]/gi,'').toLowerCase();
