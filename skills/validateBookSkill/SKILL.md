...
Run the bundled Node.js implementation for the entire verification and correction. Do not use an LLM, model API, agent-authored analysis script, Python command or external review plan to inspect, classify or correct a book. No screenshots, PDF rasterization, contact sheets or image reports are generated. Reports are `report.txt` and `report.json`.
