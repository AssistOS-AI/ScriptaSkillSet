...
The `complete` command delivers the final correction account as `RAPORT-CORECTII.txt` at the book root, beside `manifest.json`; a requested audit account goes there as `RAPORT-VERIFICARE.txt`. Final responses link these book-owned reports. Native reports, technical evidence and backups remain in the job directory; native audit-only commands retain their read-only behavior outside the job.
