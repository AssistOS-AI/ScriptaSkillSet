...
English source matching normalizes whitespace, Unicode compatibility forms, punctuation and line-end hyphenation. Unmatched lines/passages are candidates, not automatic proof of omission. Unique exact-text PDF/HTML block matches also compare relative text-size hierarchy from source line bounds and computed HTML font sizes. Unmapped geometry remains explicitly uncertified. Font inventories are compared with actual rendered families after conservative subset/variant normalization. Image inventories reveal missing/different quantities but cannot identify arbitrary PDF composites by count.
