...
test('review retains only exact recorded text differences for the same PDF',()=>{
  const f={category:'html_text_unmatched',severity:'error',excerpt:'Existing branding'};
  const review={pdfSha256:'source',differences:[{findingSha256:hash(JSON.stringify(f)),reason:'Existing editorial branding verified against source.'}]};
  assert.equal(retainReviewedDifferences([f],review,'source')[0].severity,'warning');
  assert.equal(retainReviewedDifferences([{...f,excerpt:'An actual new omission'}],review,'source')[0].severity,'error');
  assert.throws(()=>retainReviewedDifferences([f],review,'changed'),/Stale/);
});
